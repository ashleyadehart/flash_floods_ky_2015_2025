...
## This code was generated via Weather API and Google AI. 
##This code was used to retrieve weather conditions for each flash flooding event in the dataset based on the event's date and location. 
## The script uses the WeatherAPI.com (History API) to fetch daily weather data, including temperature, precipitation, wind speed, etc. 
## The retrieved weather data is then merged with the original dataset and saved as a new CSV file for further analysis. 
...
def get_historical_weather_row(session, api_key, location, date_str):
    ...
    fields = [
        "MAXTEMP_F", "MINTEMP_F", "AVGTEMP_F", "MAXWIND_MPH", "TOTALPRECIP_IN", 
        "AVGVIS_MILES", "AVGHUMIDITY", "CONDITION_TEXT", "CONDITION_CODE", 
        "UV", "DAILY_WILL_IT_RAIN", "DAILY_CHANCE_OF_RAIN"
    ]
